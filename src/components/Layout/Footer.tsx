...
const QuickLink = memo(({ icon: Icon, text, href }: {
  icon: React.ElementType;
  text: string;
  href: string;
}) => (
        ...
        <div className="hidden md:block py-4">
          <div className="grid grid-cols-12 gap-4 lg:gap-6">
            ...
            <div className="col-span-4 md:col-span-2 md:ml-8">
              <h4 className="text-[14px] font-semibold text-gray-900 mb-2">Quick Links</h4>
            ...
            <div className="col-span-4 md:col-span-2 md:ml-4">
              <h4 className="text-[14px] font-semibold text-gray-900 mb-2">Resources</h4>
              <ul className="space-y-1.5">
                ...
                <div className="flex items-center gap-4">
                  {TRUST_BADGES.map(({ icon: Icon, text, color }) => (
                    <div key={text} className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] text-gray-600 whitespace-nowrap">{text}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-3">
                  {SUPPORT_LINKS.map((link) => (
                    <a 
                      key={link}
                      href="#" 
                      className="text-[13px] text-gray-600 hover:text-blue-600 whitespace-nowrap"
                    >
                      {link}
                    </a>
                  ))}
        ...
        <div className="flex gap-4">
          <div className="flex gap-3">
            {['Latest', 'Compare', 'News', 'Guides'].map((link) => (
              <a key={link} href="#" className="text-[10px] text-gray-600">{link}</a>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {TRUST_BADGES.map(({ icon: Icon, text, color }) => (
            <div key={text} className="flex items-center gap-1">
              ...
              <span className="text-[9px] text-gray-600">{text.split(' ')[0]}</span>
            </div>
          ))}
        </div>
      </div>
